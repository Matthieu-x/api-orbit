...
async function searchTikTok(query, limit = 10) {
  ...
  while (videos.length < limit && page <= maxPages) {
    let result;
    try {
      const options = { type: "video", page };
      const cookie = process.env.TIKTOK_COOKIE || TIKTOK_COOKIE;
      if (cookie) options.cookie = cookie;
      result = await Tiktok.Search(query, options);
    } catch (error) {
    ...
    if (result?.status !== "success") {
      const hint = !(process.env.TIKTOK_COOKIE || TIKTOK_COOKIE) ? " (falta configurar la cookie de TikTok)" : "";
      debug = `La librería respondió status="${result?.status}"${result?.message ? ` con mensaje: ${result.message}` : " sin mensaje de error"}.${hint}`;
      break;
    }
